...
install_lvim () {
  ...
  if ! [ -n "$(command -v lvim)" ]; then
    : $(LV_BRANCH='release-1.3/neovim-0.9' && curl -sS https://raw.githubusercontent.com/LunarVim/LunarVim/release-1.3/neovim-0.9/utils/installer/install.sh > /tmp/lvim_install.sh && chmod +x /tmp/lvim_install.sh && bash /tmp/lvim_install.sh -- --no-install-dependencies)
    check_error
  else
    echo "skipping"
  fi
